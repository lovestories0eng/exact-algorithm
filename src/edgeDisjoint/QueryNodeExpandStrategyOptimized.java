...
public class QueryNodeExpandStrategyOptimized implements InitialGraphConstructor {
    ...
    private void createHomoGraph(MetaPath metaPath) {
        ...
        while (curHop < this.hops) {
            ...
            if (curHop == 0) initPathMapConflict(vertexMapDegrees);
            else initPathMapConflict(homoVertexMapDegrees);
    ...
    private void initPathMapConflict(HashMap<Integer, Integer> vertexMapDegrees) {
        ...
        // TODO: DELETE
        // 删除掉被剪枝掉的边
        Iterator<Map.Entry<Integer, ArrayList<ArrayList<Integer>>>> iterator = this.edgeMapPath.entrySet().iterator();
    ...
    private void homoGraphBuilder(ArrayList<Integer> path) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            if (edgeUsedTimes[vertexPairMapEdge.get(Map.entry(path.get(i), path.get(i + 1)))] == 0) System.out.println("shit");
        ...
        // TODO: DELETE PATH OF pathMapConflict
        // 路径如果被使用了则可以删掉
    ...
    private void updateVertexPairMap(ArrayList<Integer> path) {
        ...
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            for (int j = 0; j < tmpVar.size(); j++) {
                // // pathMapConflict一定包含里面的边
                // if (!pathMapConflict.containsKey(tmpVar.get(j)))
                //     System.out.println("+++++++++++++++++");
